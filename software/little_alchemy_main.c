...
#include "little_alchemy/element_list.h"
#include "little_alchemy/little_alchemy_controller.h"
#include "little_alchemy/recipes.h"
		...
		if (GetUsbTaskState() == USB_STATE_RUNNING) {
			if (!runningdebugflag) {
				...
				if (rcode == hrNAK) {
					...
					button = buf.button;
					printHex (buf.button + (buf.Xdispl<<8) + (buf.Ydispl<<16) + (buf.bytes3to7[0]<<24), 1);
				}

				if (buf.bytes3to7[0] != 0) {
					int scrollVal = buf.bytes3to7[0];
					if (scrollVal == 255) {
						scrollVal = -1;
					}
