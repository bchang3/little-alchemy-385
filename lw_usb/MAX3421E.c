...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	Status = XSpi_Transfer(&SpiInstance, tx_buf, NULL, sizeof(tx_buf));

	if (Status != XST_SUCCESS)
	{
		xil_printf ("MAXreg_wr SPI transfer failed: %d", Status);
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 0x01);
	BYTE tx_buf[nbytes + 1];

	//write reg + 2 via SPI
	tx_buf[0] = reg | 0x02;

	//write data[n] via SPI, where n goes from 0 to nbytes-1
	for (u8 byte_idx = 0; byte_idx < nbytes; byte_idx++) {
		tx_buf[byte_idx + 1] = data[byte_idx];
	}
	...
	if (Status != XST_SUCCESS)
	{
		xil_printf ("MAXbytes_wr SPI transfer failed: %d", Status);
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0x00);
	return data + nbytes;
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//read return code from SPI peripheral (see Xilinx examples)
	//if return code != 0 print an error
	Status = XSpi_Transfer(&SpiInstance, tx_buf, rx_buf, sizeof(tx_buf));

	//read return code from SPI peripheral 
	//if return code != 0 print an error
	if (Status != XST_SUCCESS)
	{
		xil_printf ("MAXreg_rd SPI transfer failed: %d", Status);
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0x00);
	//return val
	BYTE val = rx_buf[1];
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	if (Status != XST_SUCCESS)
	{
		xil_printf ("MAXbytes_rd SPI transfer failed: %d", Status);
	}


	for (u8 byte_idx = 0; byte_idx < nbytes; byte_idx++) {
		data[byte_idx] = rx_buf[byte_idx + 1];
	}
