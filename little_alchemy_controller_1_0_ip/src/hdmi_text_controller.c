...
{
	uint16_t red_nibble = (red & 0xF) << 8;
	uint16_t green_nibble = (green & 0xF) << 4;
	uint16_t blue_nibble = (blue & 0xF);
	if (color % 2 == 0) {
		hdmi_ctrl->PALETTE[color/2] = (hdmi_ctrl->PALETTE[color/2] & 0xFFFF0000) | (uint16_t)(red_nibble | green_nibble | blue_nibble);
	} else {
		hdmi_ctrl->PALETTE[color/2] = (hdmi_ctrl->PALETTE[color/2] & 0x0000FFFF) | (uint32_t)((red_nibble | green_nibble | blue_nibble) << 16);
	}
		...
		if (hdmi_ctrl->FRAME_COUNT % 30 == 0) //every 30 frames update background
			{
				...
				printf("~~~~~~~~~~~~~~~~~~~~~ \r");
				for (int byte_idx = 0; byte_idx <= strlen(dvd_string) * 2; byte_idx++) {
					if (byte_idx % 2) {
						printf("Character: %x \r", old_string[byte_idx]);
					} else {
						printf("Color: %x \r", old_string[byte_idx]);
					}
				}
				printf("~~~~~~~~~~~~~~~~~~~~~ \r");
				printf("\n \r");
